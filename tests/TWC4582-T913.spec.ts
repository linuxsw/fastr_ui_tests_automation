...
test('Bulk_Import_FDS_Mechanical_Connection_Delete (TWC4582-T913)', async ({ page }) => {
  ...
  await page.locator('#FDSMechanicalConnections_AttachBtnID').setInputFiles('./fixtures/TWC4582-T913/01. Delete a TPOC.csv');
  ...
  // Expect the TPOC data is deleted successfully in the TPOC menu
    // Select the element and retrieve its text content
  const recordMsg = await page.textContent('tr.k-grid-norecords > td');
  // Print the text content to the console
  console.log(recordMsg);

  // Create a locator for the specific element
  const noRecordsLocator = page.locator('tr.k-grid-norecords > td');
  // Use expect to assert the text content of the element
  await expect(noRecordsLocator).toHaveText('No records available');
});